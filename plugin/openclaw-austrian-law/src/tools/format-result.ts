...
export function formatLegalReviewMarkdown(review: LegalReviewResponse): string {
  ...
  const receiptLines = [
    ...
    `- **Raw Content SHA-256:** \`${receipt.raw_content_sha256 || receipt.content_sha256}\``,
    `- **Normalized Content SHA-256:** \`${receipt.normalized_content_sha256 || receipt.content_sha256}\``,
    `- **Abrufmethode:** \`${receipt.retrieval_method}\` (Cached: ${receipt.cached ? "Ja" : "Nein"})`,
